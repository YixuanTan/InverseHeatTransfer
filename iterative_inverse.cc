...
void currentsInHeater::PrintCurrents(){
  printf("\ncurrents in heaters\n");
  for(int i=0; i<currents_in_heater_.size(); i++){
    printf("%-10.3f \n", currents_in_heater_[i]);
  }
  printf("\n");
}
...
class GenerateMesh{
public:
  void CalculateCoordinates(Initialization*);
  std::vector<double> &get_x_coordinates()
    {return x_coordinates_;}
  std::vector<double> &get_y_coordinates()
    {return y_coordinates_;}
  void GenerateMeshInitializeMeshSizeInfo(Initialization*);
  void PrintCoordinatesResults(Initialization* initialization);
...
public:
  void InitializeDegreeOfFreedomAndEquationNumbers(Initialization *);
  ...
  void set_essential_bc_nodes(Initialization*, std::vector<double>&);
  void GenerateNodeAndEquationNumbersInElements(Initialization*);
  void PrintDofAndEquationNumbers(Initialization *);
...
class HalfBandWidth{
public:
  void set_accumulative_half_band_width_vector(Initialization *, DegreeOfFreedomAndEquationNumbers *);
  std::vector<int>& get_accumulative_half_band_width_vector()
    {return accumulative_half_band_width_vector_;}
...
public:
  void InitializeBoundaryCondition(Initialization *);
...
void BoundaryCondition::InitializeBoundaryCondition(Initialization *initialization){
...
public:
  void InitializeTemperatureDependentVariables(Initialization *);
  ...
  double get_heater_crosssection_area_mm_square() const
    {return heater_crosssection_area_mm_square_;}
  void PrintTemperatureDependentVariables(std::vector<int>&,Initialization *);
...
public:
  void InitializeHeaterElements(Initialization *);
  void set_elements_as_heater(Initialization *);
...
/*-----------------------------------------------------------------
------------------inverse problem(heat_matrix)---------------------
-----------------------------------------------------------------*/
  void set_element_heat_matrix();
  void MapElementalToGlobalHeatMatrix(Eigen::MatrixXd &, std::vector<int>&, int, int);
  void PrintHeatMatrix(Eigen::MatrixXd&);
  double get_resistivity(int, std::vector<int>&, std::vector<int>&, std::vector<double>&, std::vector<double>&, std::vector<double>&, Initialization *);
/*---------------------------------------------------------------*/
...
double HeaterElements::get_resistivity(int heater_number, std::vector<int>&elements_as_heater, std::vector<int>&nodes_in_elements, std::vector<double>&x_coordinates, std::vector<double>&y_coordinates, std::vector<double>&current_temperature_field, Initialization *initialization){
  int mesh_seeds_on_heater = (*((*initialization).get_mesh_parameters())).get_mesh_seeds_on_heater();
  double temperature=0.0;
  ...
  double resistivity=-6.674e-13*temperature*temperature+2.462e-9*temperature-1.893e-07;// titanium heater [Omega*m]
  return resistivity;
}
...
void HeaterElements::MapElementalToGlobalHeatMatrix(Eigen::MatrixXd &heat_matrix, std::vector<int>&equation_numbers_in_elements, const int element_number, const int heater_number){
  for(int i=0;i<Constants::kNumOfNodesInElement_;i++){
...
void HeaterElements::PrintHeatMatrix(Eigen::MatrixXd&heat_matrix){
  printf("heat_matrix is:\n");
  for(int i=0; i<heat_matrix.size()/Constants::kNumOfHeaters_; i++){
    for(int j=0; j<Constants::kNumOfHeaters_; j++){
      printf("%f  ", heat_matrix(i,j));
    }
    printf("\n");
  }
}
...
public:
  void InitializeRadiationElements(Initialization *);
  void set_elements_with_radiation(Initialization *);
...
public:
  void set_initial_temperature_field(std::vector<int>&, std::vector<double>&, Initialization *, std::vector<int>&);
...
public:
  void set_material_id_of_elements(Initialization *);
  void set_densities();
  std::vector<int>& get_material_id_of_elements()
    {return material_id_of_elements_;}
...
// printf("element number %d   x_right%f  x_left=%f\n", element_number, coordinates_in_this_element_[0][0],coordinates_in_this_element_[0][1]);
...
public:
  ...
  void set_element_body_heat_flux_tangential_matrix(int, int, std::vector<int>&, std::vector<double>&, TemperatureDependentVariables*, 
    Initialization*);
...
public:
  ...
  Eigen::MatrixXd& get_heat_matrix()
    {return heat_matrix_;}
...
void GlobalVectorsAndMatrices::InitializeGlobalVectorsAndMatrices(const int num_of_nodes, std::vector<int>&accumulative_half_band_width_vector){
  ...
  heat_matrix_.setZero(num_of_equations, Constants::kNumOfHeaters_);
}
...
public:
  ...
  double NormOfVector(Eigen::VectorXd&);
...
double Solver::NormOfVector(Eigen::VectorXd& vector_to_be_evaluated){
  double return_value=0.0;
  double summation_over_squred_components=0.0;
  for(int i=0; i<vector_to_be_evaluated.size(); i++)
    summation_over_squred_components += (vector_to_be_evaluated[i])*(vector_to_be_evaluated[i]);
  return_value = sqrt(summation_over_squred_components);
return return_value;
}
...
private:
  ...
  Eigen::MatrixXd sensitivity_matrix_;
...
void InverseAnalysisMatrices::InitializeInverseAnalysisMatrices(const int num_of_equations, const int num_of_equations_on_sample_surface, std::vector<double> &temperature_on_sample_surface, CopperSurfTemperatureDistribution *copper_surface_temperature_distribution){
  current_heat_generation_.setZero(Constants::kNumOfHeaters_);
  heat_generation_increments_.setZero(Constants::kNumOfHeaters_);
  temperature_increment_.setZero(num_of_equations_on_sample_surface);
  sensitivity_matrix_.setZero(num_of_equations, Constants::kNumOfHeaters_);
  R_matrix_.setZero(num_of_equations_on_sample_surface, num_of_equations);
  temperature_desired_.setZero(num_of_equations_on_sample_surface);
  LHS_matrix_.setZero(num_of_equations_on_sample_surface, Constants::kNumOfHeaters_);
  ...
  for(int ii=0; ii<num_of_equations_on_sample_surface; ii++)
    temperature_desired_(ii) = temperature_on_sample_surface[ii];
...
int InverseAnalysisMatrices::HeatGenerationSolver(Eigen::VectorXd &temperature_now, Solver *solver){
  ...
  error_norm_current_ = (*solver).NormOfVector(temperature_increment_);
  if( error_norm_last_ != 0.0 && error_norm_current_/error_norm_last_ > Constants::kCutoff_ && 
      error_norm_current_/error_norm_last_ <= 1.0){
  ...
  if((error_norm_current_ < Constants::kTemperatureTolerance_ || count_equal_norm_time_ == 5) && 
      error_norm_last_ != 0.0 && error_norm_current_/error_norm_last_ > Constants::kCutoff_ && error_norm_current_/error_norm_last_ <= 1.0 ){
    count_equal_norm_time_ = 0;
    return 1;
  }
  else{
...
void InverseAnalysisMatrices::UpdateCurrents(const double heater_cross_section_area_si_unit, Initialization *initialization, HeaterElements *heater_elements, std::vector<int> &elements_as_heater, std::vector<int> &nodes_in_elements, std::vector<double> &x_coordinates, std::vector<double> &y_coordinates, std::vector<double> &current_temperature_field, Solver *solver){
  current_heat_generation_ *= 1.0e6;  //  [N]/([S][mm]^2) ->  [N]/([S][M]^2) * 1.0e6  
...
void InverseAnalysisMatrices::SensitivityMatrixSolver(Eigen::MatrixXd &heat_matrix, GlobalVectorsAndMatrices *global_vectors_and_matrices){
//use LinearEquationsSolver to solve equations
  double temporary_variable = 0.0;
  double new_value = 0.0; 
  int num_of_equations = ((*global_vectors_and_matrices).get_right_hand_side_function()).size();
  int size_of_desparsed_stiffness_matrix = (*global_vectors_and_matrices).get_stiffness_matrix().size();
  sensitivity_matrix_.setZero();
...
//   decomposition 
    for(int k=0; k<num_of_equations; k++){   
      ...
      new_value=(double)sqrt( ((*global_vectors_and_matrices).JacobianMatrixIndex(k,k)) - temporary_variable);
      if(((*global_vectors_and_matrices).ModifyJacobianMatrixIndex(k,k,new_value))){
        printf("sensitivity matrix solver crashed at position 1\n");
      }
      for(int i=k+1; i<num_of_equations+1; i++){   
        temporary_variable = 0.0;   
        for(int j=0; j<k; j++){   
        ...
        new_value= (((*global_vectors_and_matrices).JacobianMatrixIndex(k,i))-temporary_variable)
                   /((*global_vectors_and_matrices).JacobianMatrixIndex(k,k));
        if(((*global_vectors_and_matrices).ModifyJacobianMatrixIndex(k,i,new_value))){
          printf("sensitivity matrix solver crashed at position 2\n");
        }   
      }   
    }   
  ...
  //  back substitution   
    new_value= ((*global_vectors_and_matrices).JacobianMatrixIndex(num_of_equations-1,num_of_equations)) 
               /((*global_vectors_and_matrices).JacobianMatrixIndex(num_of_equations-1,num_of_equations-1));
    if(((*global_vectors_and_matrices).ModifyJacobianMatrixIndex(num_of_equations-1,num_of_equations,new_value))){
      printf("sensitivity matrix solver crashed at position 3\n");
    }
    ...
    for(int k=num_of_equations-2; k>=0; k--){   
      ...
      new_value=(((*global_vectors_and_matrices).JacobianMatrixIndex(k,num_of_equations)) - temporary_variable) 
                /((*global_vectors_and_matrices).JacobianMatrixIndex(k,k)); 
      if(((*global_vectors_and_matrices).ModifyJacobianMatrixIndex(k,num_of_equations,new_value))){
        printf("sensitivity matrix solver crashed at position 4\n");
      }
    }   

  //  store disp into solution_of_last_iteration vector/
    for(int i=0; i<num_of_equations; i++){   
      sensitivity_matrix_(i, ii) = ((*global_vectors_and_matrices).JacobianMatrixIndex(i,num_of_equations));   
    }
...
/*-----------------------------inverse--------------------------------*/
  ...
  copper_surface_temperature_distribution.InitializeCopperSurfTemperatureDistribution(num_of_equations, num_of_nodes, num_of_elements, 
    &initialization);
...
//  copper_surface_temperature_distribution.PrintOutTemperature(num_of_equations);
  ...
  inverse_analysis_matrices.InitializeInverseAnalysisMatrices(num_of_equations, num_of_equations_on_sample_surface, 
    temperature_on_sample_surface, &copper_surface_temperature_distribution);
/*--------------------------------------------------------------------*/
...
/*-------------------------------inverse analysis-------------------------------------*/
    ...
    for(int i = 0; i < num_of_equations_on_sample_surface; i++){
      temperature_now(i) = global_vectors_and_matrices.get_current_temperature_field()[first_node_on_sample_surface+i];
    }

    temperature_norm_last = temperature_norm_current;
    temperature_norm_current = solver.NormOfVector(temperature_now);
...
/*--------------------------------------------------------------------------------------*/
    ...
    if(check_temperature_convergence == 1){// the desired temperature distribution has been achieved, quit the loop.
      ...
      break; //break from the for int time_step loop 
    }
